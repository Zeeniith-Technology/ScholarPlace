 ...
 * Route: /student/practice/aptitude-week-7?day=day-1|day-2|day-3|day-4|day-5
 *
 * Week 7 is a consolidation week: revision questions only, no theory, so this
 * page is the Aptitude entry point (weeks 1-6 route via a study page first).
 */
function AptitudeWeek7PracticeContent() {
    ...
    const days = {
        'day-1': { label: 'Day 1', title: 'Practice Set 1 — mixed aptitude' },
        'day-2': { label: 'Day 2', title: 'Practice Set 2 — mixed aptitude' },
        'day-3': { label: 'Day 3', title: 'Practice Set 3 — mixed aptitude' },
        'day-4': { label: 'Day 4', title: 'Practice Set 4 — mixed aptitude' },
        'day-5': { label: 'Day 5', title: 'Practice Set 5 — mixed aptitude' },
    }
    ...
    const fetchQuestions = async (dayParam: string) => {
        try {
            ...
            const response = await fetch(`${apiBaseUrl}/questions/aptitude-practice`, {
                ...
                body: JSON.stringify({ week: 7, day: parseInt(dayNum) }),
            })
                        ...
                        <div className="mb-6">
                            ...
                            <div className="flex flex-wrap gap-2">
                                {Object.entries(days).map(([key, info]) => (
                                    <button
                                        key={key}
                                        onClick={() => router.push(`/student/practice/aptitude-week-7?day=${key}`)}
...
export default function AptitudeWeek7PracticePage() {
    return (
        <Suspense fallback={<div className="flex items-center justify-center min-h-screen">
            <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-primary"></div>
        </div>}>
            <AptitudeWeek7PracticeContent />
        </Suspense>
    )
}
