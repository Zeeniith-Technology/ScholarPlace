...
/**
 * Week 2 Aptitude Learning Page
 * Interactive learning experience for Quantitative Aptitude
 * Route: /student/study/aptitude-week-2?day=day-1|day-2|day-3|day-4|day-5
 */
function AptitudeWeek2Content() {
  ...
  const days = [
    { id: 'day-1', label: 'Day 1', title: 'Percentages – Foundation & Change', dayNum: 1 },
    { id: 'day-2', label: 'Day 2', title: 'Successive Percentages & Applications', dayNum: 2 },
    { id: 'day-3', label: 'Day 3', title: 'Ratio & Proportion (Beginner Level)', dayNum: 3 },
    { id: 'day-4', label: 'Day 4', title: 'Proportion & Applications (Intermediate)', dayNum: 4 },
    { id: 'day-5', label: 'Day 5', title: 'Integrated Aptitude (Advanced Applications)', dayNum: 5 },
  ]
  ...
  const fetchStudyContent = async (day: string) => {
    try {
      ...
      const response = await fetch(`${apiBaseUrl}/syllabus/aptitude-week2-content`, {
        method: 'POST',
        credentials: 'include',
        headers: {
  ...
  const navigateToDay = (dayId: string) => {
    router.push(`/student/study/aptitude-week-2?day=${dayId}`)
  }
  ...
  const checkWeeklyTestEligibility = async () => {
    try {
      ...
      const response = await fetch(`${apiBaseUrl}/student-progress/check-weekly-test-eligibility`, {
        ...
        body: JSON.stringify({ week: 2, track: 'aptitude' }),
      })
      if (response.ok) {
  ...
  const checkBlockedRetake = async (): Promise<boolean> => {
    try {
      ...
      const response = await fetch(`${apiBaseUrl}/student-progress/check-blocked-retake`, {
        ...
        credentials: 'include',
        headers: { 'Content-Type': 'application/json', 'Authorization': authHeader },
        body: JSON.stringify({ week: 2, test_type: 'weekly' }),
      })
      if (response.ok) {
  ...
  const handleWeeklyTestClick = async () => {
    ...
    window.open('/student/aptitude/weekly/2', '_blank', 'width=1200,height=800,scrollbars=yes,resizable=yes')
  }
  ...
  const fetchBookmarks = async () => {
    try {
      ...
      const response = await fetch(`${apiBaseUrl}/student-progress/bookmarks/get`, {
        ...
        credentials: 'include',
        headers: { 'Content-Type': 'application/json', 'Authorization': authHeader },
        body: JSON.stringify({ week: 2, type: 'aptitude' }),
      })
      if (response.ok) {
        const result = await response.json()
  ...
  const saveBookmarks = async () => {
    try {
      ...
      await fetch(`${apiBaseUrl}/student-progress/bookmarks/save`, {
        ...
        credentials: 'include',
        headers: { 'Content-Type': 'application/json', 'Authorization': authHeader },
        body: JSON.stringify({ week: 2, bookmarks, type: 'aptitude' }),
      })
    } catch (error) {
  ...
  const isBookmarked = (day: string, section: string) =>
    bookmarks.some(b => b.day === day && b.section === section)
        ...
        <div className="flex h-screen overflow-hidden">
          ...
          <div className={cn(
            ...
            <div className="p-4">
              ...
              {/* Back Button */}
              <button
                onClick={() => router.push('/student/study/week-2-select')}
                    ...
                    <div className="p-6">
                      <div className="flex items-center justify-between">
                        ...
                        {(() => {
                          ...
                          return (
                            <button
                              onClick={() => router.push(`/student/practice/aptitude-week-2?day=${selectedDay}`)}
...
export default function AptitudeWeek2Page() {
  return (
    <Suspense fallback={<div className="flex items-center justify-center min-h-screen">
      <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-primary"></div>
    </div>}>
      <AptitudeWeek2Content />
    </Suspense>
  )
}
