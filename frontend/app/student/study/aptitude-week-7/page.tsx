...
/**
 * Week 7 Aptitude Learning Page
 * Consolidation week: no theory. Each day is a 50-question mixed revision set;
 * the content endpoint returns a short note rather than lesson material.
 * Interactive learning experience for Quantitative Aptitude
 * Route: /student/study/aptitude-week-7?day=day-1|day-2|day-3|day-4|day-5
 */
function AptitudeWeek7Content() {
  ...
  // Consolidation week: each day is a mixed 50-question revision set drawn from
  // all six topic areas, not a single new topic — so no per-day topic titles.
  const days = [
    { id: 'day-1', label: 'Day 1', title: 'Mixed practice set', dayNum: 1 },
    { id: 'day-2', label: 'Day 2', title: 'Mixed practice set', dayNum: 2 },
    { id: 'day-3', label: 'Day 3', title: 'Mixed practice set', dayNum: 3 },
    { id: 'day-4', label: 'Day 4', title: 'Mixed practice set', dayNum: 4 },
    { id: 'day-5', label: 'Day 5', title: 'Mixed practice set', dayNum: 5 },
  ]
  ...
  const fetchStudyContent = async (day: string) => {
    try {
      ...
      const response = await fetch(`${apiBaseUrl}/syllabus/aptitude-week7-content`, {
        method: 'POST',
        credentials: 'include',
        headers: {
  ...
  const navigateToDay = (dayId: string) => {
    router.push(`/student/study/aptitude-week-7?day=${dayId}`)
  }
  ...
  const checkWeeklyTestEligibility = async () => {
    try {
      ...
      const response = await fetch(`${apiBaseUrl}/student-progress/check-weekly-test-eligibility`, {
        ...
        body: JSON.stringify({ week: 7, track: 'aptitude' }),
      })
      if (response.ok) {
  ...
  const checkBlockedRetake = async (): Promise<boolean> => {
    try {
      ...
      const response = await fetch(`${apiBaseUrl}/student-progress/check-blocked-retake`, {
        ...
        credentials: 'include',
        headers: { 'Content-Type': 'application/json', 'Authorization': authHeader },
        body: JSON.stringify({ week: 7, test_type: 'weekly' }),
      })
      if (response.ok) {
  ...
  const handleWeeklyTestClick = async () => {
    ...
    window.open('/student/aptitude/weekly/7', '_blank', 'width=1200,height=800,scrollbars=yes,resizable=yes')
  }
  ...
  const fetchBookmarks = async () => {
    try {
      ...
      const response = await fetch(`${apiBaseUrl}/student-progress/bookmarks/get`, {
        ...
        credentials: 'include',
        headers: { 'Content-Type': 'application/json', 'Authorization': authHeader },
        body: JSON.stringify({ week: 7, type: 'aptitude' }),
      })
      if (response.ok) {
        const result = await response.json()
  ...
  const saveBookmarks = async () => {
    try {
      ...
      await fetch(`${apiBaseUrl}/student-progress/bookmarks/save`, {
        ...
        credentials: 'include',
        headers: { 'Content-Type': 'application/json', 'Authorization': authHeader },
        body: JSON.stringify({ week: 7, bookmarks, type: 'aptitude' }),
      })
        ...
        <div className="flex h-screen overflow-hidden">
          ...
          <div className={cn(
            ...
            <div className="p-4">
              ...
              {/* Back Button */}
              <button
                onClick={() => router.push('/student/study/week-7-select')}
              ...
              {selectedDay === 'day-5' && (
                <div className="mt-6 pt-6 border-t border-neutral-light/20">
                  ...
                  <button onClick={handleWeeklyTestClick}
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <div className="font-semibold text-sm">Week 7 Aptitude Test</div>
                        </div>
                    ...
                    <div className="p-6">
                      <div className="flex items-center justify-between">
                        ...
                        {(() => {
                          ...
                          return (
                            <button
                              onClick={() => router.push(`/student/practice/aptitude-week-7?day=${selectedDay}`)}
...
export default function AptitudeWeek7Page() {
  return (
    <Suspense fallback={<div className="flex items-center justify-center min-h-screen">
      <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-primary"></div>
    </div>}>
      <AptitudeWeek7Content />
    </Suspense>
  )
}
