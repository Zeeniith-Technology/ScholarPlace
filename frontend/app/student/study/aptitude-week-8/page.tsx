 ...
 * Route: /student/study/aptitude-week-8?day=day-1|day-2|day-3|day-4|day-5
 */
function AptitudeWeek8Content() {
  ...
  const fetchStudyContent = async (day: string) => {
    try {
      ...
      const response = await fetch(`${apiBaseUrl}/syllabus/aptitude-week8-content`, {
        method: 'POST',
        credentials: 'include',
        headers: {
  ...
  const navigateToDay = (dayId: string) => {
    router.push(`/student/study/aptitude-week-8?day=${dayId}`)
  }
  ...
  const checkWeeklyTestEligibility = async () => {
    try {
      ...
      const response = await fetch(`${apiBaseUrl}/student-progress/check-weekly-test-eligibility`, {
        ...
        body: JSON.stringify({ week: 8, track: 'aptitude' }),
      })
      if (response.ok) {
  ...
  const checkBlockedRetake = async (): Promise<boolean> => {
    try {
      ...
      const response = await fetch(`${apiBaseUrl}/student-progress/check-blocked-retake`, {
        ...
        credentials: 'include',
        headers: { 'Content-Type': 'application/json', 'Authorization': authHeader },
        body: JSON.stringify({ week: 8, test_type: 'weekly' }),
      })
      if (response.ok) {
  ...
  const handleWeeklyTestClick = async () => {
    ...
    window.open('/student/aptitude/weekly/8', '_blank', 'width=1200,height=800,scrollbars=yes,resizable=yes')
  }
  ...
  const fetchBookmarks = async () => {
    try {
      ...
      const response = await fetch(`${apiBaseUrl}/student-progress/bookmarks/get`, {
        ...
        credentials: 'include',
        headers: { 'Content-Type': 'application/json', 'Authorization': authHeader },
        body: JSON.stringify({ week: 8, type: 'aptitude' }),
      })
      if (response.ok) {
        const result = await response.json()
  ...
  const saveBookmarks = async () => {
    try {
      ...
      await fetch(`${apiBaseUrl}/student-progress/bookmarks/save`, {
        ...
        credentials: 'include',
        headers: { 'Content-Type': 'application/json', 'Authorization': authHeader },
        body: JSON.stringify({ week: 8, bookmarks, type: 'aptitude' }),
      })
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <div className="font-semibold text-sm">Week 8 Aptitude Test</div>
                        </div>
                    ...
                    <div className="p-6">
                      <div className="flex items-center justify-between">
                        ...
                        {(() => {
                          ...
                          return (
                            <button
                              onClick={() => router.push(`/student/practice/aptitude-week-8?day=${selectedDay}`)}
...
export default function AptitudeWeek8Page() {
  return (
    <Suspense fallback={<div className="flex items-center justify-center min-h-screen">
      <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-primary"></div>
    </div>}>
      <AptitudeWeek8Content />
    </Suspense>
  )
}
