...
/**
 * Week 6 Aptitude Learning Page
 * Interactive learning experience for Quantitative Aptitude
 * Route: /student/study/aptitude-week-6?day=day-1|day-2|day-3|day-4|day-5
 */
function AptitudeWeek6Content() {
  ...
  const days = [
    { id: 'day-1', label: 'Day 1', title: 'Profit & Loss Fundamentals', dayNum: 1 },
    { id: 'day-2', label: 'Day 2', title: 'Advanced P&L - Discounts & Marked Price', dayNum: 2 },
    { id: 'day-3', label: 'Day 3', title: 'Simple Interest Complete Mastery', dayNum: 3 },
    { id: 'day-4', label: 'Day 4', title: 'Compound Interest Advanced + Effective Rates', dayNum: 4 },
    { id: 'day-5', label: 'Day 5', title: 'Complete Integration - Business Scenarios', dayNum: 5 },
  ]
  ...
  const fetchStudyContent = async (day: string) => {
    try {
      ...
      const response = await fetch(`${apiBaseUrl}/syllabus/aptitude-week6-content`, {
        method: 'POST',
        credentials: 'include',
        headers: {
  ...
  const navigateToDay = (dayId: string) => {
    router.push(`/student/study/aptitude-week-6?day=${dayId}`)
  }
  ...
  const checkWeeklyTestEligibility = async () => {
    try {
      ...
      const response = await fetch(`${apiBaseUrl}/student-progress/check-weekly-test-eligibility`, {
        ...
        body: JSON.stringify({ week: 6, track: 'aptitude' }),
      })
      if (response.ok) {
  ...
  const checkBlockedRetake = async (): Promise<boolean> => {
    try {
      ...
      const response = await fetch(`${apiBaseUrl}/student-progress/check-blocked-retake`, {
        ...
        credentials: 'include',
        headers: { 'Content-Type': 'application/json', 'Authorization': authHeader },
        body: JSON.stringify({ week: 6, test_type: 'weekly' }),
      })
      if (response.ok) {
  ...
  const handleWeeklyTestClick = async () => {
    ...
    window.open('/student/aptitude/weekly/6', '_blank', 'width=1200,height=800,scrollbars=yes,resizable=yes')
  }
  ...
  const fetchBookmarks = async () => {
    try {
      ...
      const response = await fetch(`${apiBaseUrl}/student-progress/bookmarks/get`, {
        ...
        credentials: 'include',
        headers: { 'Content-Type': 'application/json', 'Authorization': authHeader },
        body: JSON.stringify({ week: 6, type: 'aptitude' }),
      })
      if (response.ok) {
        const result = await response.json()
  ...
  const saveBookmarks = async () => {
    try {
      ...
      await fetch(`${apiBaseUrl}/student-progress/bookmarks/save`, {
        ...
        credentials: 'include',
        headers: { 'Content-Type': 'application/json', 'Authorization': authHeader },
        body: JSON.stringify({ week: 6, bookmarks, type: 'aptitude' }),
      })
        ...
        <div className="flex h-screen overflow-hidden">
          ...
          <div className={cn(
            ...
            <div className="p-4">
              ...
              {/* Back Button */}
              <button
                onClick={() => router.push('/student/study/week-6-select')}
              ...
              {selectedDay === 'day-5' && (
                <div className="mt-6 pt-6 border-t border-neutral-light/20">
                  ...
                  <button onClick={handleWeeklyTestClick} className={`w-full text-left p-3 rounded-lg transition-all ${weeklyTestEligibility?.eligible ? 'bg-secondary/10 hover:bg-secondary/20 border-2 border-secondary/30 cursor-pointer' : 'bg-neutral-light/10 border-2 border-neutral-light/20 opacity-60 cursor-pointer'}`}>
                    <div className="flex items-center justify-between">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          <Trophy className={`w-4 h-4 ${weeklyTestEligibility?.eligible ? 'text-secondary' : 'text-neutral-light'}`} />
                          <div className="font-semibold text-sm">Week 6 Aptitude Test</div>
                        </div>
                    ...
                    <div className="p-6">
                      <div className="flex items-center justify-between">
                        ...
                        {(() => {
                          ...
                          return (
                            <button
                              onClick={() => router.push(`/student/practice/aptitude-week-6?day=${selectedDay}`)}
...
export default function AptitudeWeek6Page() {
  return (
    <Suspense fallback={<div className="flex items-center justify-center min-h-screen">
      <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-primary"></div>
    </div>}>
      <AptitudeWeek6Content />
    </Suspense>
  )
}
