 ...
 * Manage and view all students in the department/college
 * Route: /dept-tpc/students
 */
export default function DepartmentTPCStudentsPage() {
  ...
  const checkAuth = async () => {
    try {
      ...
      // Only allow DeptTPC to access this page
      if (!profileResult.success || userRole?.toLowerCase() !== 'depttpc') {
      ...
      console.error('[DeptTPC Students] Auth verification error:', error)
  ...
  const fetchStudents = async () => {
    try {
      ...
      const response = await fetch(`${apiBaseUrl}/tpc-dept/students/list`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': authHeader,
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm sm:text-base text-neutral-light">
              View and manage all students in your {userInfo?.role === 'TPC' ? 'college' : 'department'}
            </p>
          </div>
          <button
