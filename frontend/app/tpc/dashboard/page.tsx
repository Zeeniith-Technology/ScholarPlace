...
/**
 * TPC Dashboard Page
 * Admin dashboard for College TPC to manage students and track performance
 * Route: /tpc/dashboard
 */
export default function TPCDashboardPage() {
  ...
  const checkAuth = async () => {
    try {
      ...
      // Only allow TPC to access this dashboard
      if (!profileResult.success || normalizedRole !== 'tpc') {
        if (normalizedRole === 'depttpc' || normalizedRole === 'dept-tpc') {
      ...
      console.error('[TPC Dashboard] Auth verification error:', error)
  ...
  const fetchDepartments = async () => {
    try {
      ...
      if (data.success) {
        ...
        console.error('[TPC Dashboard] Error fetching departments:', error)
      }
    }
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      // Fetch dashboard stats (TPC endpoint)
      const statsResponse = await fetch(`${apiBaseUrl}/tpc-college/dashboard/stats`, {
        method: 'POST',
        headers,
      ...
      const topResponse = await fetch(`${apiBaseUrl}/tpc-college/students/top-performers`, {
        method: 'POST',
        headers,
        body: JSON.stringify({ limit: 5, ...requestBody }),
      })
      ...
      const attentionResponse = await fetch(`${apiBaseUrl}/tpc-college/students/needs-attention`, {
        method: 'POST',
        headers,
        body: JSON.stringify(requestBody),
      })
      ...
      console.error('[TPC Dashboard] Error fetching dashboard data:', error)
          ...
          <div className="lg:col-span-2">
            <Card className="transition-all duration-300 hover:shadow-lg animate-smooth-appear" style={{ animationDelay: '400ms' }}>
              <div className="flex items-center justify-between mb-4 sm:mb-6">
                ...
                <Link
                  href="/tpc/students"
                  className="text-xs sm:text-sm text-primary hover:underline font-medium flex items-center gap-1 transition-all duration-200 hover:gap-2"
                >
                  View All
          ...
          <Link href="/tpc/students">
            <Card className="group transition-all duration-300 hover:shadow-lg hover:-translate-y-0.5 cursor-pointer animate-smooth-appear" style={{ animationDelay: '800ms' }}>
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-semibold text-neutral text-sm sm:text-base">Manage Students</h3>
          ...
          <Link href="/tpc/tests">
            <Card className="group transition-all duration-300 hover:shadow-lg hover:-translate-y-0.5 cursor-pointer animate-smooth-appear" style={{ animationDelay: '900ms' }}>
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-semibold text-neutral text-sm sm:text-base">Manage Tests</h3>
          ...
          <Link href="/tpc/reports">
            <Card className="group transition-all duration-300 hover:shadow-lg hover:-translate-y-0.5 cursor-pointer animate-smooth-appear" style={{ animationDelay: '1100ms' }}>
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-semibold text-neutral text-sm sm:text-base">Generate Reports</h3>
