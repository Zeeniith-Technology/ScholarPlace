...
/**
 * TPC Settings Page
 * College and account settings
 * Route: /tpc/settings
 */
export default function TPCSettingsPage() {
  ...
  const fetchProfile = async () => {
    try {
      ...
      if (data.success && data.data) {
        ...
        setFormData({
          name: data.data.name || data.data.person_name || '',
          email: data.data.email || data.data.person_email || '',
          phone: data.data.phone || data.data.person_phone || data.data.contact_number || '',
  ...
  const handleSaveProfile = async () => {
    try {
      ...
      if (data.success) {
        ...
        if (data.data) {
          setFormData({
            name: data.data.name || data.data.person_name || formData.name,
            email: data.data.email || data.data.person_email || formData.email,
            phone: data.data.phone || data.data.person_phone || data.data.contact_number || formData.phone,
                    ...
                    <div className="pt-4 border-t border-gray-200">
                      <div className="flex items-center gap-2 text-sm">
                        ...
                        <Badge variant="primary" className="text-xs">
                          {profile.role || profile.person_role || 'TPC'}
                        </Badge>
                      </div>
                    </div>
                  )}
